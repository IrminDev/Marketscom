      ...
      <div class="flex justify-between py-5 w-full px-5 border-b-2 border-slate-800">
        <div class="flex gap-x-4">
          ...
          <div class="min-w-0 flex-auto">
            <Link className=' text-slate-300 transition-all ease-linear hover:text-blue-600 font-semibold' href={`./admin/profile/${props.user.id}`}>{`${props.user.nombre}`} {`${props.user.apellido}`}</Link>
        ...
        <div className='hidden sm:flex sm:flex-row sm:items-center'>
          <Link className=' text-slate-300 transition-all ease-linear hover:text-blue-600 font-semibold' href={`./admin/reports/${props.user.id}`}>Reportes</Link>
        </div>
        <div className='hidden sm:flex sm:flex-row sm:items-center'>
