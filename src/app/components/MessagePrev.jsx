...
// Mensaje de previsualizacion, recibe como parametro user (usuario listado) y userMain (usuario logueado)
// SE MODIFICARA A SOLO TOMAR LOS IDs Y HACER EL FECTH DESDE EL COMPONENTE
const MessagePrev = (props) => {
    // Hacer fetch del ultimo mensaje
  return (
    <Link className=' flex items-center w-full justify-between rounded-lg mt-3 pr-4 bg-slate-900 border-2 border-slate-800' href={`./user/chat/${props.user.id}`}>
